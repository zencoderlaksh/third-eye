...
const DEFAULT_BACK_COLOR = { r: 0, g: 0, b: 0 };
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!context || !context.ext.formatRGBA) return;
    ...
    function generateColor() {
      if (!RAINBOW_MODE && COLOR) {
        const base = hexToRgbNormalized(COLOR);
        const jitter = 0.9 + Math.random() * 0.2;
        return {
          r: base.r * 0.18 * jitter,
          g: base.g * 0.18 * jitter,
          b: base.b * 0.18 * jitter,
        };
      }
    ...
    function update() {
      const now = performance.now();
      if (now - lastInputTime > 2200) {
        // Fluid has dissipated; clear and sleep until next pointer move
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
